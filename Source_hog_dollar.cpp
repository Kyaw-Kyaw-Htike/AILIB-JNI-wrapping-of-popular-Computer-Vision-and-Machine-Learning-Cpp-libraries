...
/*
I is the pointer to image (linear array in column major).
h and w is the height and width of the image.
d is the dimension of the image. d=1 means only one channel. d=3 means 3 channels
M is the pointer to magnitude image output (linear array in column major)
O is the pointer to orientation image output (linear array in column major)
full: if true compute angles in [0,2*pi) else in [0,pi). Default should be false
*/
